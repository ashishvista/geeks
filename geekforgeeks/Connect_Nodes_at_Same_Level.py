...
#
# def connect(root):
#     qnodes = collections.deque([])
#     qnodes.append(Qnode(root, 0))
#
#     while qnodes:
#         item = qnodes.popleft()
#         if qnodes:
#             itemRight = qnodes[0]
#             if item.i == itemRight.i:
#                 item.node.nextRight = itemRight.node
#         if item.node.left:
#             qnodes.append(Qnode(item.node.left, item.i + 1))
#         if item.node.right:
#             qnodes.append(Qnode(item.node.right, item.i + 1))
...
def connect(root):
    ...
    while qnodes:
        ...
        itemRight = qnodes[0]
        if item:
            item.nextRight = itemRight
            if item.left:
                qnodes.append(item.left)
            if item.right:
                qnodes.append(item.right)
        else:
            qnodes.append(None)
